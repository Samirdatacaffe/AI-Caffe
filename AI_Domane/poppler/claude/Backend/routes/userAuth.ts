...
// ==================== CHECK EMAIL EXISTS ====================
router.post('/check-email', authLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/login', authLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const lIp = (req.headers['x-forwarded-for'] as string)?.split(',')[0]?.trim() || req.ip || '';
    const refreshHash = await bcrypt.hash(refreshToken, 10);
    await prisma.session.create({
      data: {
        userId: user.id,
...
router.get('/work', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {

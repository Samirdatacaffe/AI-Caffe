...
async function resolveLocation(ip: string | null): Promise<{ location: string; publicIp: string | null }> {
  try {
    let lookupIp = ip;
    ...
    if (!lookupIp) return { location: 'Unknown', publicIp: null };
...
export const createSession = async (
  ...
  await db.query(
    `INSERT INTO user_sessions (user_id, refresh_token_hash, expires_at, device, ip_address, location, last_active_at)
     VALUES ($1, $2, $3, $4, $5, $6, NOW())`,
    [userId, hash, expiresAt, device, ipAddress, location]
  );
};
...
export const getActiveSessions = async (userId: string): Promise<ActiveSession[]> => {
  if (useMemory) {
    const sessions = memoryStore.sessions.get(userId) || [];
    return sessions
      .filter((s) => new Date(s.expires_at as string) > new Date())
      .map((s) => ({
        id: s.id as string,
        ...
        last_active_at: new Date((s.last_active_at || s.created_at) as string),
      }));
  }

  const result = await db.query(
    `SELECT id, device, location, ip_address, created_at, last_active_at
     FROM user_sessions
     WHERE user_id = $1 AND expires_at > NOW()
...
export const deleteSessionById = async (userId: string, sessionId: string): Promise<void> => {
  if (useMemory) {
    const sessions = memoryStore.sessions.get(userId) || [];
...
export const deleteSession = async (userId: string, refreshToken: string): Promise<void> => {
  if (useMemory) {
    const sessions = memoryStore.sessions.get(userId) || [];
    for (let i = 0; i < sessions.length; i++) {
      if (await bcrypt.compare(refreshToken, sessions[i].refresh_token_hash as string)) {
        sessions.splice(i, 1);
        break;
      }
    }

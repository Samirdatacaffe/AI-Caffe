...
let useDb = false;
let otpDb: { query: (text: string, params?: unknown[]) => Promise<{ rows: Record<string, unknown>[]; rowCount: number | null }> };

// In-memory fallback
const memoryOtps = new Map<string, OtpRecord>();

/** Initialize: try DB, fall back to memory */
export const initOtpStorage = async (): Promise<void> => {
  try {
...
const dbUpsertOtp = async (email: string, otpHash: string, expiryTime: Date): Promise<void> => {
  await otpDb.query('DELETE FROM otp_service.email_otps WHERE email = $1', [email]);
  await otpDb.query(
    `INSERT INTO otp_service.email_otps (email, otp_hash, is_verified, expiry_time, attempts, created_at, updated_at)
     VALUES ($1, $2, FALSE, $3, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
    [email, otpHash, expiryTime]
  );
};

const dbGetOtp = async (email: string): Promise<OtpRecord | null> => {
  const result = await otpDb.query(
    `SELECT email, otp_hash, is_verified, expiry_time, attempts
     FROM otp_service.email_otps WHERE email = $1
     ORDER BY created_at DESC LIMIT 1`,
    [email]
  );
  return result.rows.length > 0 ? result.rows[0] as unknown as OtpRecord : null;
};

const dbIncrementAttempts = async (email: string): Promise<void> => {
  await otpDb.query(
    `UPDATE otp_service.email_otps SET attempts = attempts + 1, updated_at = CURRENT_TIMESTAMP WHERE email = $1`,
    [email]
  );
};

const dbMarkVerified = async (email: string): Promise<void> => {
  await otpDb.query(
    `UPDATE otp_service.email_otps SET is_verified = TRUE, updated_at = CURRENT_TIMESTAMP WHERE email = $1`,
    [email]
  );
};

const dbDeleteOtp = async (email: string): Promise<void> => {
  await otpDb.query('DELETE FROM otp_service.email_otps WHERE email = $1', [email]);
};

// ==================== Memory Operations ====================

const memUpsertOtp = (email: string, otpHash: string, expiryTime: Date): void => {
  memoryOtps.set(email, { email, otp_hash: otpHash, is_verified: false, expiry_time: expiryTime, attempts: 0 });
};

const memGetOtp = (email: string): OtpRecord | null => {
  return memoryOtps.get(email) ?? null;
};

// ==================== Public API ====================

export const sendOtpToEmail = async (email: string): Promise<OtpResult> => {
...
export const sendOtpToMultiple = async (emails: string[]): Promise<OtpResult[]> => {
  const unique = [...new Set(emails.map(e => e.trim().toLowerCase()))];
  return Promise.all(unique.map(sendOtpToEmail));
};

export const verifyOtp = async (email: string, otp: string): Promise<VerifyResult> => {
  const normalized = email.trim().toLowerCase();
  ...
  try {
    ...
    if (new Date(record.expiry_time) < new Date()) {
      ...
      if (useDb) await dbDeleteOtp(normalized); else memoryOtps.delete(normalized);
...
export const cleanupExpiredOtps = async (): Promise<number> => {
  if (useDb) {
    try {
      const result = await otpDb.query(
        'DELETE FROM otp_service.email_otps WHERE expiry_time < CURRENT_TIMESTAMP'
      );
      const count = result.rowCount ?? 0;
      if (count > 0) logger.info('OTP_CLEANUP', `Deleted ${count} expired OTP(s)`);
      return count;
    } catch (err) {
  ...
  if (count > 0) logger.info('OTP_CLEANUP', `Deleted ${count} expired OTP(s) from memory`);
  return count;
};
